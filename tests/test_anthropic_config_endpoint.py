...
class TestAnthropicConfigEndpoint:
    def test_get_returns_defaults_when_unset(self, superadmin_client) -> None:
        r = superadmin_client.get("/admin/anthropic-config")
    ...
    def test_patch_overrides_base_url_and_model(self, superadmin_client) -> None:
        r = superadmin_client.patch(
    ...
    def test_blank_base_url_falls_back_to_default(self, superadmin_client) -> None:
        ...
        # Then clear it.
        r = superadmin_client.patch("/admin/anthropic-config", json={"base_url": ""})
        assert r.status_code == 200
        # resolve_anthropic_config returns the default when stored value is blank.
        assert r.json()["base_url"] == "https://api.anthropic.com"

    def test_api_key_dash_clears_it(self, superadmin_client) -> None:
        superadmin_client.patch("/admin/anthropic-config", json={"api_key": "sk-test-key"})
        r = superadmin_client.get("/admin/anthropic-config")
        assert r.json()["has_api_key"] is True

        superadmin_client.patch("/admin/anthropic-config", json={"api_key": "-"})
        r = superadmin_client.get("/admin/anthropic-config")
        assert r.json()["has_api_key"] is False

    def test_api_key_empty_keeps_existing(self, superadmin_client) -> None:
        superadmin_client.patch("/admin/anthropic-config", json={"api_key": "sk-keep-this"})
        # Empty string in the PATCH must NOT clear the key.
        superadmin_client.patch("/admin/anthropic-config", json={"api_key": ""})
        r = superadmin_client.get("/admin/anthropic-config")
        assert r.json()["has_api_key"] is True

    def test_updating_anthropic_does_not_change_active_provider(self, superadmin_client) -> None:
        ...
        before = superadmin_client.get("/admin/ai-config").json()["provider"]
        # Update only the Anthropic config.
        superadmin_client.patch(
            "/admin/anthropic-config",
            json={"base_url": "https://my-proxy.test/v1", "model": "claude-haiku-4-5"},
        )
        after = superadmin_client.get("/admin/ai-config").json()["provider"]
        assert after == before, (
