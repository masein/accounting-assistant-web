...
MATRIX = [
    ...
    # Invoice e-mail + quotes (roadmap §4.2): automatic customer mail is owner-level.
    ("PUT", "/invoices/reminder-settings", {O}),
    ("GET", "/invoices/reminder-settings", {O, C, A, V, P}),
    ("POST", "/invoices/{invoice_id}/send", {O, C, A, P}),
    ("GET", "/invoices/{invoice_id}/emails", {O, C, A, V, P}),
    ("POST", "/quotes", {O, C, A, P}),
    ("POST", "/quotes/{quote_id}/convert", {O, C, A, P}),
    ("GET", "/quotes", {O, C, A, V, P}),
    ("POST", "/recurring-invoices", {O, C, A, P}),
    ("PATCH", "/recurring-invoices/{template_id}", {O, C, A, P}),
    ("GET", "/recurring-invoices", {O, C, A, V, P}),
    ("PUT", "/moadian/settings", {O}),
    ("POST", "/moadian/export", {O, C, A, P}),
    ("PATCH", "/moadian/invoices/{invoice_id}", {O, C, A, P}),
    ("GET", "/moadian/invoices", {O, C, A, V, P}),
